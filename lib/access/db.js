...
module.exports = function (nimda) {
  ...
  function connect(verbose = true) {
    return new Promise((resolve, reject) => {
      ...
      if (verbose) { nimda.util.log.task('Connecting to database..', 0) }
      MongoClient.connect(nimda.config.mongo.url)
      .then(result => {
        db = result
        if (verbose) { nimda.util.log.task('Connected', 1) }
        resolve(db)
      })
      .catch(err => {
        if (verbose) { nimda.util.log.task('Failed to connect to database', 2) }
        return reject(err)
      })
    })
  }
