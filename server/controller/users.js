...
module.exports = function (nimda) {
   ...
   */
  function * list() {
    yield nimda.util.orm.getPaginated.call(this, 'users', 'users', {}, user => {
      return {
        id       : user.id,
        groups   : user.groups,
  ...
  function * update() {
    ...
    for (let key in validator.schema) {
      ...
      if (key === 'password') {
        changes[key] = yield nimda.orm.users.hashPassword(val)
    ...
    user = yield nimda.db.update('users', { _id: this.user._id }, { $set: changes })
    ...
    if (passwordChanged) {
      ...
      if (
        passwordChanged &&
        !(yield nimda.orm.users.matchPasswords(oldPassword, this.user.password))
      ) {
