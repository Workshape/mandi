...
module.exports = function (nimda) {
  ...
  function * update() {
    ...
    yield Promise.map(Object.keys(values), key => {
      return nimda.db.findOne('statics', { key }).then(entry => {
        let value = values[key]

        // Override existing ones
        if (entry) {
          return nimda.db.update('statics', { _id: entry._id }, { $set: { value } })
        }

        // Store new ones
        return nimda.db.insert('statics', { key, value })
      })
    })
